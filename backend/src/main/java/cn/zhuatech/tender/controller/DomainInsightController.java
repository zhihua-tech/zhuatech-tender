...
import cn.zhuatech.tender.service.DomainInsightService;
...
public DomainInsightController(DomainInsightService service){this.service=service;}
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @PostMapping("/tender") ApiResponse<Map<String,Object>> analyze(@Valid @RequestBody DomainInsightService.InsightRequest request){return ApiResponse.ok(service.analyze(request));}
}
